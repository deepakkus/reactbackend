...
const QrcodeTicket = require("../../Models/qrcodeTicket");
...
const create = async (req, res) => {
  const v = new Validator(req.body, {
    ...
    return event
        .save()
        .then(async (data) => {
            const mail = Email.findOne({title: "User Event Registration" }, function (err, docs) {
                if (err){
                    console.log(err)
                }
                else{
                    console.log("Result : ", docs.subject);
                    sendEmail(docs.email,docs.content)
                }
            });
    ...
    async (err, data) => {
      if (err) {
        ...
        if (req.body.tickets && req.body.tickets.length > 0) {
          ...
          let delprev = await Ticket.deleteMany({ eventId: data._id });
          if (delprev) {
            let createTicket = await createTicketMany(req, res);
            if (!createTicket) {
              delete data.tickets;
  ...
  return Event.aggregate([
    ...
    .then(async (data) => {
      if (data && data.length > 0) {
        ...
        for (
          let index = 0;
          index < data[0]?.tablelistsData?.tableList?.length;
          index++
        ) {
          const element = data[0]?.tablelistsData?.tableList[index];
          let dat = await CheckAvailableTable({
        ...
        for (let index = 0; index < data[0]?.tickets?.length; index++) {
          ...
          let dat = await CheckAvailableTicket({
            eventId: req.params.id,
            tickets: [element],
          });
...
const GetPromotionsCode = async (req, res) => {
  return Event.aggregate([
    ...
    .then((data) => {
      ...
      if (
        promotions &&
        promotions.filter((it) => it.code == req.body.code).length > 0
      ) {
        res.status(200).json({
          status: false,
          message: "code already exists",
      ...
      },
      {
        $lookup: {
          ...
          pipeline: [
            ...
            {
              $match: {
                newEndDate: { $gte: new Date() },
                newStartDate: { $lte: new Date() },
              },
            },
          ],
          as: "eventsData",
...
const QrBookingNew = async (req, res) => {
  const { type, id, bookingId } = req.body;
  if (type == "ticket") {
    return QrcodeTicket.aggregate([
      {
        $match: {
      ...
      .then((data) => {
        ...
        return res.status(200).json({
          status: true,
          message: "Get All Ticket Details Successfully",
          data: data[0],
        });
      })
      .catch((error) => {
    ...
    return eventBooking
      ...
      .then((data) => {
        ...
        return res.status(200).json({
          ...
          message: "Get All Event Booking his Successfully",
          data: {
            eventbookingsData: data[0],
            scanStatus: data[0]?.organizatiStatus == "pending" ? true : false,
          },
        });
      })
      .catch((error) => {
...
const UpdateQqScan = async (req, res) => {
  const { type, id } = req.body;
  if (type == "ticket") {
    return QrcodeTicket.findOneAndUpdate(
      { _id: mongoose.Types.ObjectId(id) },
      { scanStatus: false },
      async (err, data) => {
        if (err) {
    ...
    return QrcodeMenu.findOneAndUpdate(
      { _id: mongoose.Types.ObjectId(id) },
      { scanStatus: false },
      async (err, data) => {
        if (err) {
